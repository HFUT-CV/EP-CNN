...
class Darknet(nn.Module):
    ...
    def forward(self, x, var=None):
        img_size = x.shape[-1]
        layer_outputs = []          # 经过每个层得到的输出组成的列表
        output = []
        modelzip = zip(self.module_defs, self.module_list)
        for i, (module_def, module) in enumerate(modelzip):
        # for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype in ['convolutional', 'upsample', 'maxpool']:
                ...
                output.append(x)
            # draw_features(32, 32, x.cpu().numpy(), "{}/f6_layer2.png".format(i))
